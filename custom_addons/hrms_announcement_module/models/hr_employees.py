...
class HrEmployees(models.Model):
    ...
    _inherit = 'hr.employee.masterlist'
    ...
    def _compute_announcement_count(self):
        ...
        for employee in self:
            announcement_ids_general = self.env[
                'hr.announcements'].sudo().search_count(
                [('is_announcement', '=', True),
                 ('state', 'in', ('approved', 'done')),
                 ('date_start', '<=', fields.Date.today())])
            announcement_ids_emp = (self.env['hr.announcements'].
            sudo().search_count(
                [('employee_ids', 'in', self.id),
                 ('state', 'in', ('approved', 'done')),
                 ('date_start', '<=', fields.Date.today())]))
            announcement_ids_dep = (self.env['hr.announcements'].
            sudo().search_count(
                [('department_ids', 'in', self.department_id.id),
                 ('state', 'in', ('approved', 'done')),
                 ('date_start', '<=', fields.Date.today())]))
            announcement_ids_job = (self.env['hr.announcements'].
            sudo().search_count(
                [('position_ids', 'in', self.job_id.id),
                 ('state', 'in', ('approved', 'done')),
                 ('date_start', '<=', fields.Date.today())]))
            employee.announcements_count = (announcement_ids_general +
    ...
    def action_open_announcements(self):
        ...
        announcement_ids_general = self.env[
            'hr.announcements'].sudo().search(
            [('is_announcement', '=', True),
             ('state', 'in', ('approved', 'done')),
             ('date_start', '<=', fields.Date.today())])
        announcement_ids_emp = self.env['hr.announcements'].sudo().search(
            [('employee_ids', 'in', self.id),
             ('state', 'in', ('approved', 'done')),
             ('date_start', '<=', fields.Date.today())])
        announcement_ids_dep = self.env['hr.announcements'].sudo().search(
            [('department_ids', 'in', self.department_id.id),
             ('state', 'in', ('approved', 'done')),
             ('date_start', '<=', fields.Date.today())])
        announcement_ids_job = self.env['hr.announcements'].sudo().search(
            [('position_ids', 'in', self.job_id.id),
             ('state', 'in', ('approved', 'done')),
             ('date_start', '<=', fields.Date.today())])
